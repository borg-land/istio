...
mkdir -p /var/lib/istio/ztunnel
mkdir -p /var/lib/istio/pilot-agent
mkdir -p /var/lib/istio/certs
mkdir -p /var/log/istio

chown -R istio-proxy.istio-proxy /var/lib/istio/ /var/log/istio
chmod o+rx /usr/local/bin/{ztunnel,pilot-agent,istio-start.sh}

# pilot-agent and ztunnel may run with effective uid 0 in order to run ztunnel with
...
chmod 2755 /usr/local/bin/{ztunnel,pilot-agent}
