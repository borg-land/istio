...
func (s *Server) Stop(skipCleanup bool) {
	...
	// If we are being upgraded, we may want to stop and leave dataplane hooks in place
	// (to be reconciled by our replacement)
	if !skipCleanup {
		log.Info("CNI ambient server terminating, cleaning up node net rules")
		// Under any other context, just do what we normally do and unhook the dataplane
		s.dataplane.Stop()
	}
}

func (s *Server) ShouldStopForUpgrade(selfName, selfNamespace string) bool {
	cniCM, err := s.kubeClient.Kube().CoreV1().ConfigMaps(selfNamespace).Get(context.Background(), fmt.Sprintf("%s-config", selfName), metav1.GetOptions{})

	// Check the (randomly-generated by Helm at templating time) slug value in the configmap.
	//
	// if the value currently in the CM doesn't match the value we were initially seeded with via env,
	// that means the agent is being upgraded - someone "reinstalled" the chart and regenerated the configmap
	// since this container started.
	//
	// If it does match, that means a new instance of the chart has not been installed since this container started,
	// so we just uninstall normally.
	if err == nil && cniCM.Data != nil {
		cmVer := cniCM.Data[pconstants.IstioCniAgentVersionEnv]
		localVer := os.Getenv(pconstants.IstioCniAgentVersionEnv)
		if localVer != cmVer {
			// Bail, the version in the CM does not equal the version we have -
			// which means the CNI CM has been changed since we started.
			// A CNI upgrade is in progress, do not unhook dataplane, just stop
			log.Infof("different version: our version %s, configmap version %s, this is an upgrade, leaving plugin", localVer, cmVer)
			return true
		}
		// If the version has not changed under us, this is a regular pod restart with no CM change,
		// OR a removal, unhook.
		//
		// TODO we currently have no way to tell the difference between "pod killed because remove"
		// and "pod crashed or was killed manually", so we cannot leave the plugin in place for the latter case.
		log.Info("same version, this is a restart/replace, removing plugin")
	} else {
		// If the CM is gone, it's definitely not an upgrade, so carry on like normal.
		log.Debugf("error getting CM, this is not an upgrade %s", err)
	}
